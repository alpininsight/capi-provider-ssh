...
| Conditions and lifecycle readiness | Legacy CAPI condition conventions | Conditions are recommended; compatible fields are accepted. A `Ready` condition covers the full lifecycle, including deletion [S1], [S2], [S3] | **Implemented reporting in current source:** cleanup makes readiness false, pause and cleanup conditions are explicit, `observedGeneration` is recorded and transition times change only with condition status. Unrelated conditions are preserved. These optional field improvements do not establish v1beta2 conformance. [Status semantics](api-reference.md#lifecycle-conditions) |
| Failure reporting and remediation | `failureReason` / `failureMessage` historically signalled terminal failures | Conditions and documented consumer policy replace special terminal-failure handling. Current CAPI no longer uses those legacy fields to trigger MachineHealthCheck remediation [S1], [S3] | **Partial:** cleanup failure/quarantine now updates lifecycle conditions and preserves claims; failed post-cleanup release cannot erase success. An automatic CAPI remediation outcome remains unproven. [Cleanup](../python/capi_provider_ssh/lifecycle.py) |
...
| Pause and lifecycle safety | Pause, identity and lifecycle coordination already apply | Pause remains recommended; a `Paused` condition is recommended when pause is implemented [S1], [S2] | **Implemented behavior and reporting:** owner-chain pause/UID checks and True/False/Unknown conditions exist; ordinary pause preserves prior readiness. Keep and retest peering, Leases, remote fencing and durable receipts through migration. [Contracts](../python/capi_provider_ssh/contracts.py), [status semantics](api-reference.md#lifecycle-conditions) |
...
| Test and component baseline | Existing harness exercises legacy CAPI APIs and references | Acceptance must prove the advertised contract and migration path, including independence from retiring APIs | **Implemented legacy baseline / open v1beta2 evidence:** CAPI/CABPK/KCP 1.12.11, Kubernetes 1.34.11, four Ready nodes, bootstrap/deletion/reuse/failover; test counts are revision-specific. No explicit v1beta2 lane exists. CAPI 1.12 is a maintenance/transition baseline; 1.14.1 was the latest stable release checked on this review date. [Harness pins](../python/tests/kind/upstream.json), [tests](testing.md), [S6], [S7] |

The initial condition findings are corrected in this source. The presence of a
...
3. **Complete status behavior — provider maintainer.** The current source now
   has consistent condition updates for initialization, normal reconciliation, pause, deletion,
   ...
   Remaining migration acceptance: prove those semantics through the v1beta2
   consumer lane. Current `Ready`/`Paused` reporting must match the operation, cleanup
