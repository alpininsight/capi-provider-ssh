...
| Governance — rules configured, reviewer continuity still requires action | One independent approval, CODEOWNER review, last-push approval, stale-review dismissal and resolved threads complement the eight strict checks. The old SRE CODEOWNER was invalid; the source now names the verified administrator | An administrator-authored PR requires a second authorized eligible reviewer. Its identity/access must be supplied by the repository owner; no self-approval or bypass is defined. Verify base-branch CODEOWNERS and effective rules via [CI governance](ci-governance.md) |
