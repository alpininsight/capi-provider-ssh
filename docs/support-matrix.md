...
| Provider memory | The old live deployment retains its previous resource budget until the next GitOps rollout | 128 MiB request / 512 MiB limit, Burstable; small SDK-free in-cluster HTTPS probe | A post-merge concurrent-probe failure reproduced cgroup OOM kills at the old 256 MiB limit. TLS/token and concurrent-probe/OOM checks now cover this gap; a higher limit is demand-based headroom, not automatic VPA resizing |
