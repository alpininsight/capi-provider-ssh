...
from capi_provider_ssh.api_io import run_api
from capi_provider_ssh.conditions import condition, patch_conditions, report_pause
...
def _reconcile(spec: dict, name: str, namespace: str, meta: dict, patch: kopf.Patch, status=None) -> None:
...
async def sshcluster_create(spec, name, namespace, meta, patch, status=None, **_kwargs):
    """Handle SSHCluster creation."""
    logger.info("SSHCluster %s/%s created", namespace, name)
    await run_api(_reconcile, spec, name, namespace, meta, patch, status)
...
async def sshcluster_update(spec, name, namespace, meta, patch, status=None, **_kwargs):
...
async def sshcluster_delete(name, namespace, patch=None, spec=None, meta=None, status=None, **_kwargs):
    """Handle SSHCluster deletion -- no-op cleanup, respecting CAPI pause."""
    patch = patch if patch is not None else kopf.Patch()
    status = status or {}
    meta = meta or {}
    patch.status["ready"] = False
    patch_conditions(
        ...
        meta,
        [
            condition("Ready", "False", "Deleting", "Cluster infrastructure is deleting"),
            condition("InfrastructureReady", "False", "Deleting", "Cluster infrastructure is deleting"),
            condition("CleanupSucceeded", "False", "Deleting", "Cluster deletion is awaiting pause checks"),
        ],
    )
    if await run_api(report_pause, patch, status, meta, lambda: is_paused(spec or {}, meta, namespace)):
    ...
    patch_conditions(
        ...
        meta,
        [
            condition("ControlPlaneEndpointReady", "False", "Deleting", "Control plane endpoint is being removed"),
            condition(
                "CleanupSucceeded",
                "True",
                "CleanupNotRequired",
                "SSHCluster has no provider-owned resources to clean up",
            ),
        ],
    )
